...
import { STUDENT_GET_CLASS } from "@/components/api";
                        ...
                        <div className="flex flex-row mt-4 w-full gap-4 justify-center items-center">
                            {quizData.map((quiz, index) => (
                                <div key={index} className="flex flex-col w-fit border-b-2 bg-white border border-[#cdcdcd] p-2 rounded-2xl">
                                    ...
                                    <Link href={`/s/course/${classroomId}/quiz/${quiz["quizId"]}`} className="mt-2 bg-gray-200 text-black px-4 py-2 rounded-2xl border w-full text-center border-[#cdcdcd] hover:cursor-pointer">Take Quiz</Link>
                                </div>
                            ))}
                        </div>
                    </div>
