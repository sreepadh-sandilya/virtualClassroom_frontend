        ...
        <>
            ...
            {isLoading || data === null || !data.length > 0 ? <LoadingScreen /> : (
                <main className="flex flex-1 flex-col mt-24 w-[80%] ml-auto mr-auto items-center justify-center">
                    ...
                    <Link href={`/d/course/${courseId}/add-class`} className="mt-4 bg-gray-200 text-black px-4 py-2 rounded-2xl border w-[32%] text-center border-[#cdcdcd] hover:cursor-pointer">Add New Class</Link>
                                    ...
                                    {facData[key].map((classroom) => {
                                        return (
                                            <div key={classroom["classroomId"]} className="bg-white border border-[#cdcdcd] p-4 rounded-xl">
                                                ...
                                                <Link href={encodeURI(`/d/course/${courseId}/${classroom["classroomId"]}`)} className="mt-4 bg-gray-200 text-black px-4 py-2 rounded-2xl border w-full text-center border-[#cdcdcd] hover:cursor-pointer mr-2">View Class</Link>
                                                <Link href={encodeURI(`/d/course/${courseId}/edit-class?batchStart=${classroom['batchStart']}&batchEnd=${classroom['batchEnd']}&managerEmail=${classroom["managerEmail"]}&section=${classroom["section"]}&isMentor=${classroom["isMentor"]}`)} className="mt-4 bg-gray-200 text-black px-4 py-2 rounded-2xl border w-full text-center border-[#cdcdcd] hover:cursor-pointer">Edit Class</Link>
                                            </div>
                                        )
                                    })}
                                </div>
