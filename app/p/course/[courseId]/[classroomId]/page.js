            ...
            {isLoading || data === null ? <LoadingScreen /> : (
                <main className="flex flex-1 flex-col mt-24 w-[80%] ml-auto mr-auto items-center justify-center">
                    ...
                    <div className="flex flex-col mt-8 w-full justify-center items-center">
                        ...
                        <Link href={`/p/course/${courseId}/${classroomId}/schedule-class`} className="mt-4 bg-gray-200 text-black px-4 py-2 rounded-2xl border w-fit text-center border-[#cdcdcd] hover:cursor-pointer">Schedule Class</Link>
                        ...
                        <div className="flex flex-row mt-4 w-full gap-4 justify-center items-center">
                            {classRoomData.map((classData, index) => (
                                <div key={index} className="flex flex-col w-fit border-b-2 bg-white border border-[#cdcdcd] p-2 rounded-2xl">
                                    ...
                                    <Link href={
                                        `/p/course/${courseId}/${classroomId}/edit-schedule?classId=${classData["classId"]}&classStartTime=${classData["classStartTime"]}&classEndTime=${classData["classEndTime"]}&classLink=${classData["classLink"]}`
                    ...
                    <div className="flex flex-col mt-16 w-full justify-center items-center mb-32">
                        ...
                        {/* New Quiz */}
                        <Link href={`/p/course/${courseId}/${classroomId}/quiz/new`} className="mt-4 bg-gray-200 text-black px-4 py-2 rounded-2xl border w-fit text-center border-[#cdcdcd] hover:cursor-pointer">New Quiz</Link>
                        ...
                        <div className="flex flex-row mt-4 w-full gap-4 justify-center items-center">
                            {quizData.map((quiz, index) => (
                                <div key={index} className="flex flex-col w-fit border-b-2 bg-white border border-[#cdcdcd] p-2 rounded-2xl">
                                    ...
                                    <Link href={`/p/course/${courseId}/${classroomId}/quiz/${quiz["quizId"]}`} className="mt-2 bg-gray-200 text-black px-4 py-2 rounded-2xl border w-full text-center border-[#cdcdcd] hover:cursor-pointer">View Quiz</Link>
                                </div>
                            ))}
                        </div>
                    </div>
